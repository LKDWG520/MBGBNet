...
class Down(nn.Sequential):
    def __init__(self, in_channels, out_channels):
        super(Down, self).__init__(
            nn.MaxPool2d(2, stride=2), #最大池化的窗口为2，对邻域内特征点取最大
...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True):  #bilinear双线性插值
        super(Up, self).__init__()
        if bilinear:
        #scale_factor指定输出为输入的多少倍数，mode可使用的上采样算法，如果为True，输入的角像素将与输出张量对齐，因此将保存下来这些像素的值
